...
@section('content')
    ...
    <form action="{{route('vendor.post-vendor-user',['id'=>$dataUser->id ?? -1])}}" method="post" class="input-has-icon">
        @csrf
        <div class="row">
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    @if(!empty($dataUser->business_areas))
                        @php
                            $business = json_decode($dataUser->business_areas);
                        @endphp
                    <input type="checkbox" name="business_areas[]" value="tour" {{(in_array('tour',$business) ? 'checked' : '')}}><span>Tour</span>
                    <input type="checkbox" name="business_areas[]" value="car" {{(in_array('car',$business) ? 'checked' : '')}}><span>Xe</span>
                    <input type="checkbox" name="business_areas[]" value="flight" {{(in_array('flight',$business) ? 'checked' : '')}}><span>Vé máy bay</span>
                    <input type="checkbox" name="business_areas[]" value="hotel" {{(in_array('hotel',$business) ? 'checked' : '')}}><span>Khách sạn</span>
                    <input type="checkbox" name="business_areas[]" value="event" {{(in_array('service',$business) ? 'checked' : '')}}><span>Tiện ích</span>
                        @else
                        ...
                        <input type="checkbox" name="business_areas[]" value="event"><span>Tiện ích</span>
                    @endif
                </div>
                <div class="form-group">
                    <label>Quyền</label>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        <option value="">{{__('-- Select --')}}</option>
                        <option value="publish" {{($dataUser && $dataUser->status=='publish') ? 'selected' : '' }}>Đang sử dụng</option>
                        <option value="blocked" {{($dataUser && $dataUser->status=='blocked') ? 'selected' : '' }}>Khóa tài khoản</option>
                    </select>

...
Route::group(['prefix'=>'user/'.config('tour.tour_route_prefix'),'middleware' => ['auth','verified']],function(){
    ...
    Route::get('/schedule/{target_id}','ManageTourController@Tourschedule')->name('tour.vendor.schedule');
    Route::get('/schedule-create/{target_id}','ManageTourController@createSchedule')->name('tour.vendor.schedulecreate');
    Route::get('/schedule/{target_id}/edit/{id}','ManageTourController@editSchedule')->name('tour.vendor.scheduleedit');
    Route::post('/schedule-store/{target_id}','ManageTourController@storeSchedule')->name('tour.vendor.storeschedule');
});

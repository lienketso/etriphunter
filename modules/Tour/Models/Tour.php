    ...
    public function addToCart(Request $request)
    {
        ...
        if ($meta) {
            ...
            if(!empty($meta->deposit)){
                $meta_deposit = $meta->deposit;
                $percent_deposit = $meta->deposit['percent'];
            }
            // for Availability Calendar
        ...
        $total_deposit = $total * ($percent_deposit/100);
        if($total_deposit<=0){
            $total_deposit = null;
        }
        ...
        //update lại commission theo sản phẩm
        if(!is_null($this->commission)) {
            $commission = json_decode($this->commission);
            $total_commission = $commission->adult + $commission->child + $commission->young + $commission->baby;
            $commission_type = $this->commission;
        }else{
            $total_commission = 0;
            $commission_type = '{"adult":"0","child":"0","young":"0","baby":"0"}';
        }
        $booking->commission = $total_commission;
        $booking->commission_type = $commission_type;
//        Hàm commmission mặc định
//        $booking->calculateCommission();
        ...
        if ($check) {
            ...
            $booking->addMeta('deposit',$meta_deposit);
            if ($this->isDepositEnable()) {
...
//            $model_Tour->where("bravo_tours.location_id", $location_id);
            $model_Tour->where("bravo_tours.muti_location","like","%{$location_id}%");
        }
        if (!empty($start_location_id = $request->query('start_location_id'))){
            $model_Tour->where("bravo_tours.start_location_id", $start_location_id);
        }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        return $model_Tour->with([
            ...
            'translations',
            'tourDate'=>function($e){
                return $e->where('active',1)->whereDate('start_date','>=',Carbon::today());
            }
        ])->paginate($limit);
    }
    ...
    public function tourDate(){
        return $this->hasMany(TourDate::class,'target_id','id');
    }

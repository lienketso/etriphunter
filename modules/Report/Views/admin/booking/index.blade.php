                    ...
                    <table class="table table-hover bravo-list-item">
                        ...
                        @foreach($rows as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    <strong>Tổng : {{format_money_main($booking->commission)}}</strong>
                                    @if(!is_null($infoCommission))
                                    <ul>
                                        <li>Người lớn : {{format_money_main($infoCommission->adult)}}</li>
                                        <li>Trẻ em : {{format_money_main($infoCommission->child)}}</li>
                                        <li>Trẻ nhỏ : {{format_money_main($infoCommission->young)}}</li>
                                        <li>Em bé : {{format_money_main($infoCommission->baby)}}</li>
                                    </ul>
                                    @if(!is_null($row->customer_id))
                                        ...
                                        @endphp
                                        @if($user->is_agency==1 && $row->status=='completed')
                                            <div class="agency-money">
                                                <p>Hoa hồng cho đại lý :  </p>
                                                <p><strong>{{format_money_main($agency_money)}}</strong></p>
                                            </div>
                                            @if(is_null($row->is_refund_wallet))
                                                <div class="agency-money" style="padding: 10px 0">
                                                    <a href="{{route('report.admin.accept-commission',$row->id)}}"
                                            ...
                                            <div class="agency-money">
                                                ...
                                                <p style="color: #902b2b"><strong>{{format_money_main($row->privilege_amount)}}</strong></p>
                                            </div>
                                            <hr/>
                                            <div class="agency-money">
                                                <p>Lợi nhuận :  </p>
                                                <p style="color: #902b2b"><strong>{{format_money_main(($row->commission - $agency_money) - $row->privilege_amount)}}</strong></p>
                                            </div>
                                        @endif
                                        @endif
                                        @endif
                                ...
                                <td>
                                    <span class="badge badge-{{$row->status}}">{{$row->statusName}}</span>
                                </td>
                                <td>

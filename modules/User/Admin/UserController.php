...
class UserController extends AdminController
{
    ...
    public function store(Request $request, $id)
    {
        if($id and $id>0){
            ...
            $privilege= Privilege::where('id',$request->privilege_id)->first();
            $num=User::where('privilege_id', $request->privilege_id)->count();
            if($row->privilege_id!= $request->privilege_id && $num >= $privilege->max_user){
                $this->checkPermission('privilege_update');
                return back()->withInput($request->input())->with('error', __('Max user reached'));
            }
        ...
        $row->privilege_id = $request->input('privilege_id');
        $row->privilege_available = $request->input('privilege_available');
        $row->company_id = $request->input('company_id');
        if(!is_null($request->input('business_areas'))){
    ...
    public function getUserImport(Request $request){
        return view('User::admin.import');
    }
    public function userImportRequest(Request $request){
        Excel::import(new UsersImport, $request->file);
        return redirect()->route('user.admin.index')->with('success', 'User Imported Successfully');
    }

    public function getUserAgency(){
        $rows = User::where('agency_type','!=',null)->where('file_agency','!=',null)->paginate(20);
        return view('User::admin.upgrade-agency',compact('rows'));
    }
    public function getUserAgencyUpgrade(Request $request,$id){
        $user = User::find($id);
        if(!$user){
            return redirect(404);
        }
        $user->is_agency = 1;
        $user->create_user = Auth::id();
        $user->save();
        return redirect()->back()->with('success','Bạn đã chấp nhận đại lý');
    }

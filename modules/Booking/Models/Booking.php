...
class Booking extends BaseModel
{
    ...
    public function hotel(){
        return $this->hasOne(Hotel::class,'id','object_id');
    }
    public function flight(){
        return $this->hasOne(Flight::class,'id','object_id');
    }
    public function event(){
        return $this->hasOne(\Modules\Event\Models\Event::class,'id','object_id');
    }
    ...
    public static function getStatisticChartData($from, $to, $statuses = false, $customer_id = false, $vendor_id = false)
    {
        ...
        $sql_raw[] = 'sum( `agency_amount` ) AS agency_amount';
        $sql_raw[] = 'sum( `privilege_amount` ) AS privilege_amount';
        if ($statuses) {
        ...
        if (($to - $from) / DAY_IN_SECONDS > 90) {
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                $data['detail']['total_for_vendor']['val'] += $dataBooking->total_price - ( $dataBooking->total_commission + $dataBooking->total_fees );
                $data['detail']['total_for_agency']['val'] += $dataBooking->agency_amount;
                $data['detail']['total_for_member']['val'] += $dataBooking->privilege_amount;
                if ($statuses) {
                    foreach ($statuses as $status) {
        ...
        $data['detail']['total_for_vendor']['val'] = format_money_main($data['detail']['total_for_vendor']['val']);
        $data['detail']['total_for_agency']['val'] = format_money_main($data['detail']['total_for_agency']['val']);
        $data['detail']['total_for_member']['val'] = format_money_main($data['detail']['total_for_member']['val']);
        return $data;
    }
    ...
    //chỉnh sửa lại commission từ sản phẩm
    ...
    public function reloadCalculateTotalBooking(){
        ...
        $total_booking = $total_booking - ($this->coupon_amount + $this->privilege_amount);
        if($total_booking < 0 ){
            $total_booking = 0;
        }
...
//        $this->calculateCommission();
    ...
    public function ValidatePrivilege(){
        ...
        if (new \DateTime($user->privilege_available) < new \DateTime('now')){
            return false;
        }
        if($privilege->status != 'publish'){
            return false;
        }
        if($this->getMeta('guests') > $privilege-> max_user){
            return false;
        }
        if($this->object_model != 'privilege'){
            if($privilege->count){
                if($privilege->count <=  $user->privilege_count){
                    return false;
                }
            }           
        }

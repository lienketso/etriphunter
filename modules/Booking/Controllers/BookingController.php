...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    public function checkout($code)
    {
        ...
        if(!is_enable_guest_checkout() ) {
            ...
            if ($booking->ValidatePrivilege()) {
                if ($booking->object_model != 'privilege') {
                    $booking->privilege_amount = ($booking->total) * ($privilege->discount) / 100;
                    if ($booking->privilege_amount > ($privilege->amount - $user->privilege_amount)) {
                        $booking->privilege_amount = $privilege->amount - $user->privilege_amount;
                    }
                } else {
                    if ($privilege) {
                        $booking->privilege_amount = $privilege->price;
                    } else {
                        $booking->privilege_amount = 0;
                    }
                }
            } else {
                $booking->privilege_amount = 0;
            }
            $booking->total -= $booking->privilege_amount;
        }
    ...
    public function doCheckout(Request $request)
    {
        ...
        //Member list
        $booking->members = json_encode($request->input('members'));
        //privilege
        if(!is_enable_guest_checkout()) {
            $privilege = Privilege::where('id', $user->privilege_id)->first();
            if ($booking->ValidatePrivilege()) {
        ...
        if (Auth::check()) {
            ...
            $user->privilege_amount +=  $booking->privilege_amount;
            $user -> privilege_count += 1;
            $user->save();
        }
        ...
        $text = "Thông báo dịch vụ mới được đặt : #ETRIP".$booking->id."\n";
        $text .= "Ngày đăng ký : <b>".showVNdate($booking->created_at)."</b>\n";
        $text .= "Người liên hệ : <b>".$booking->first_namename." ".$booking->last_name."</b>\n";
        $text .= "Số điện thoại : <b>".$booking->phone ."</b>\n";
        $text .= "Ngày đi : <b>".showVNdate($booking->start_date) ."</b>\n";
        $text .= "Ngày về : <b>".showVNdate($booking->end_date) ."</b>\n";
        $object_model = $booking->object_model;
        if($object_model=='tour'){
            $text .= "Tên dịch vụ : <b>".$booking->tour->name ."</b>\n";
        }elseif ($object_model=='car'){
            $text .= "Tên dịch vụ : <b>".$booking->car->name ."</b>\n";
        }elseif ($object_model=='hotel'){
            $text .= "Tên dịch vụ : <b>".$booking->hotel->name ."</b>\n";
        }elseif ($object_model=='flight'){
            $text .= "Tên dịch vụ : <b>".$booking->flight->name ."</b>\n";
        }elseif ($object_model=='event'){
            $text .= "Tên dịch vụ : <b>".$booking->event->name ."</b>\n";
        }
        $text .= "Giá trị : <b>".format_money_main($booking->pay_now) ."</b>\n";
        $text .= "Số lượng khách : <b>".$booking->total_guests."</b>\n";
        $text .= "Phương thức thanh toán : <b>".$booking->gateway."</b>\n";
        if($booking->vendor_id){
            $vendor = $booking->vendor->name;
        }else{
            $vendor = "Chưa rõ nhà cung cấp";
        }
        $text .= "Nhà cung cấp : <b>".$vendor."</b>\n";
        $text .= "Để xem chi tiết đăng nhập : <b>".env('APP_URL', '')."</b>\n";

...
<div class="row">
    <div class="col-sm-4">
        <h3 class="form-group-title">{{__('Success Request Tour')}}</h3>
        <p class="form-group-desc">{{__('Notice of tour booking on demand successfully')}}</p>
    </div>
    <div class="col-sm-8">
        <div class="panel">
            <div class="form-group">
                <label class="" style="padding: 10px">{{__("Successful tour booking information")}}</label>
                <div class="form-controls">
                    <textarea name="tour_request_success" class="d-none has-ckeditor" cols="30" rows="10">{{setting_item_with_lang('tour_request_success',request()->query('lang')) }}</textarea>
                </div>
            </div>
        </div>
    </div>

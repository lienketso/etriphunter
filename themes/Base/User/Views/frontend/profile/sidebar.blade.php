<div class="profile-summary mb-2">
    <div class="profile-avatar">
            ...
            <div class="avatar-img avatar-cover" style="background-image: url('{{(!is_null($user->company_id)) ? get_file_url($user->company->logo) : $user->getAvatarUrl()}}')">
            </div>

    </div>
    <div class="text-center mb-1"><span class="role-name  badge badge-primary">{{$user->role_name}}</span></div>
    <h3 class="display-name">{{(!is_null($user->company_id)) ? $user->company->name : $user->business_name }}
        @if($user->is_verified)

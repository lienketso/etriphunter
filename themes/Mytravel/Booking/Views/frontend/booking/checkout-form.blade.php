                                ...
                                @if(!empty($person_types))
                                    @foreach($person_types as $key => $type)
                                        @php
                                            $arr = $arr + $type['number'];
                                        @endphp
{{--                                        <div class="item-member-tour">--}}
{{--                                            <p>{{$type['name_'.$lang_local] ?? __($type['name'])}} - {{$type['desc_'.$lang_local] ?? __($type['desc'])}}</p>--}}
{{--                                        </div>--}}
                                    @endforeach
                                        @for($i=1;$i<=$arr;$i++)
                                            <div class="item-f-member">
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-6">
                                                        ...
                                                        <input type="text" class="form-control" value="{{old('members')}}" name="members[{{$i}}][name]" placeholder="Nguyen Van A">
                                                    </div>
                                                    <div class="col-lg-5">
                                                        <label>{{__('Date of birth')}}</label>
                                                        <input type="text" class="form-control" value="{{old('members')}}" name="members[{{$i}}][birthday]" placeholder="15/07/1989">
                                                    </div>
                                                </div>
                                            </div>
                                        @endfor
                ...
                <div class="col-sm-12 mb-4">
                    @if(!empty($booking->getJsonMeta('deposit')))
                    <div class="deposit-option">
                        @php $deposit = $booking->getJsonMeta('deposit')@endphp
                        <div class="info-deposit">
                            <p style="margin-bottom: 0">Đặt cọc tối thiểu : {{$deposit['percent']}} % giá trị dịch vụ</p>
                            <p>{{ $deposit['date_duration'] }}</p>
                        </div>
                    </div>
                    @endif

        ...
        <div class="bravo-list-item">
            <div class="d-flex justify-content-between align-items-center mb-4 topbar-search">
                <h3 class="font-size-21 font-weight-bold mb-0 text-lh-1">
                    @if($rows->total() > 1)
                        {{ __(":count utilities found",['count'=>$rows->total()]) }}
                    @else
                        {{ __(":count utilities found",['count'=>$rows->total()]) }}
                    @endif
                </h3>
            ...
            <div class="list-item">
                <div class="row">
                    ...
                    @else
                        <div class="col-lg-12">
                            {{__("Utilities not found")}}
                        </div>
                    @endif
                </div>
            </div>
            @if($rows->total() > 0)
                <div class="text-center text-md-left font-size-14 mb-3 text-lh-1">{{ __("Showing :from - :to of :total utilities",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</div>
            @endif
